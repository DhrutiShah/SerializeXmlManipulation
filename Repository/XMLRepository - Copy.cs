    ...
    public class XMLRepository<TEntity> : IXMLRepository<TEntity> where TEntity : class
    {
      ...
      //  private TDataSource list;
        ...
        public XMLRepository(string filename)
        {
            ...
            {
                _fileName = filename;
                _serializer = new XmlSerializer(typeof(TDataSource));
                FileInfo fInfo = new FileInfo(_fileName);
                if (fInfo.Exists)
                {
                    using (FileStream myWriter = File.OpenRead(_fileName))
                    {
                        list = (TDataSource)_serializer.Deserialize(myWriter);
                        myWriter.Close();
                    }
                }
                else
                {
                    list = Activator.CreateInstance<TDataSource>();
                }
            }
            catch (Exception ex)
            {
